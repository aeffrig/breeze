 ...
 */
@SerialVersionUID(1)
class SparseVector[@spec(Byte, Short, Double, Int, Float, Long) V](val array: SparseArray[V])
...
object SparseVector extends SparseVectorOps
  ...
  def zeros[@spec(Byte, Short, Double, Int, Float, Long) V: ClassTag:Zero](size: Int) = new SparseVector(Array.empty, Array.empty[V], 0, size)
  def apply[@spec(Byte, Short, Double, Int, Float, Long) V:Zero](values: Array[V]) = new SparseVector(Array.range(0,values.length), values, values.length, values.length)

  def apply[V:ClassTag:Zero](values: V*):SparseVector[V] = apply(values.toArray)
  def fill[@spec(Byte, Short, Double, Int, Float, Long) V:ClassTag:Zero](size: Int)(v: =>V):SparseVector[V] = apply(Array.fill(size)(v))
  def tabulate[@spec(Byte, Short, Double, Int, Float, Long) V:ClassTag:Zero](size: Int)(f: Int=>V):SparseVector[V]= apply(Array.tabulate(size)(f))
  ...
  // implicits
  class CanCopySparseVector[@spec(Byte, Short, Double, Int, Float, Long) V:ClassTag:Zero] extends CanCopy[SparseVector[V]] {
    def apply(v1: SparseVector[V]) = {
      v1.copy
    }
  }

  implicit def canCopySparse[@spec(Byte, Short, Double, Int, Float, Long) V: ClassTag: Zero] = new CanCopySparseVector[V]
