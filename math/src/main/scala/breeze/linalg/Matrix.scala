 ...
 * @author dlwh
 */
trait MatrixLike[@spec(Byte, Short, Double, Int, Float, Long) V, +Self  <: Matrix[V]] extends Tensor[(Int, Int), V] with TensorLike[(Int, Int), V, Self] {
  def map[V2, That](fn: V=>V2)(implicit canMapValues: CanMapValues[Self @uncheckedVariance , V, V2, That]):That = values map fn

}

trait Matrix[@spec(Byte, Short, Double, Int, Float, Long) V] extends MatrixLike[V, Matrix[V]] {
...
object Matrix extends MatrixConstructors[Matrix]
  ...
  def zeros[@spec(Byte, Short, Double, Int, Float, Long) V: ClassTag:Zero](rows: Int, cols: Int): Matrix[V] = DenseMatrix.zeros(rows, cols)

  def create[@spec(Byte, Short, Double, Int, Float, Long) V:Zero](rows: Int, cols: Int, data: Array[V]): Matrix[V] = DenseMatrix.create(rows, cols, data)
...
trait MatrixConstructors[Mat[T]<:Matrix[T]] {
  def zeros[@spec(Byte, Short, Double, Int, Float, Long) V:ClassTag:Zero](rows: Int, cols: Int):Mat[V]
  def create[@spec(Byte, Short, Double, Int, Float, Long) V:Zero](rows: Int, cols: Int, data: Array[V]):Mat[V]
   ...
   * @return
   */
  def ones[@spec(Byte, Short, Double, Int, Float, Long) V:ClassTag:Zero:Semiring](rows: Int, cols: Int):Mat[V] = {
    fill(rows,cols)(implicitly[Semiring[V]].one)
  }

  def fill[@spec(Byte, Short, Double, Int, Float, Long) V:ClassTag:Zero](rows: Int, cols: Int)(v: =>V):Mat[V] = create(rows, cols, Array.fill(rows * cols)(v))
  def tabulate[@spec(Byte, Short, Double, Int, Float, Long) V:ClassTag:Zero](rows: Int, cols: Int)(f: (Int,Int)=>V):Mat[V]= {
  ...
  def apply[@specialized(/* Don't remove until SI-8886 is closed*/) R,
            @spec(Byte, Short, Double, Int, Float, Long) V](rows : R*)(implicit rl : LiteralRow[R,V], man : ClassTag[V], zero: Zero[V]) = {
